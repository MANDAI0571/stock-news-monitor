...
> **【2026-08-23 追記】ChatGPT(Codex)の300万円運用は廃止しました。**
> 高重さんの指示により、`chatgpt_300man_note.py`、`data/chatgpt_300man_*`、
> `docs/codex_300man_ledger.md`、`.github/workflows/chatgpt-300man-open-fill.yml` を削除しました。
> note下書きは3本（52週新高値／押し目／300万円Claude）になりました。
> 本書の「5. 触ってはいけないもの」にある **「Codexの300万円運用3ファイルは削除・置換しない」**
> は、この指示により失効しています。以下は2026-07-20時点の合意の記録として残します。
...
- ~~Codexの**300万円運用3ファイルは削除・置換しない**。~~（2026-08-23 高重さんの指示で削除済み。本書冒頭の追記を参照）
