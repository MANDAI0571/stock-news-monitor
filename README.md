...
- Gmail通知: `.github/workflows/intraday_high_alert.yml` の直近高値接近・到達アラートだけ
  ...
  - schedule実行はGmail送信ON、手動実行は `send_mail=true` を選んだ場合だけ送信
  - 同じ銘柄・同じアラート種別は、GitHub Actions cacheの当日stateで1日1回に抑制
  - 必要なGitHub Secrets: `GMAIL_USER` / `GMAIL_APP_PASSWORD` / `MAIL_TO`
- 送らないメール:
  - note下書き完了通知
  - 300万円運用通知
  - 52週高値更新/接近通知
